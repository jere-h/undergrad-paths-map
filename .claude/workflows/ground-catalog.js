...
5. Recommend a first-paint preselect for the app tab: 4-6 input ids that converge on one career while keeping others visibly open (breadth plus a couple of committal picks), with the exact command to apply it: node scripts/register-catalog.mjs --id ${cfg.industry} --label ${JSON.stringify(cfg.industryLabel)} --module ./${cfg.industry}.js --note "<keep the existing note>" --preselect "<id1,id2,...>".
